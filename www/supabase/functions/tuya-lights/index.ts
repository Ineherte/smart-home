...
async function getTuyaToken(): Promise<{ access_token: string; expires_in: number; refresh_token?: string }> {
  ...
  const sign = await hmacSha256(clientSecret, `${clientId}${timestamp}${stringToSign}`);
...
async function sendTuyaCommand(token: string, deviceId: string, action: TuyaAction, deviceCode: string) {
  const clientId = Deno.env.get('TUYA_ACCESS_ID');
  const clientSecret = Deno.env.get('TUYA_ACCESS_SECRET');
  const apiHost = Deno.env.get('TUYA_API_HOST') || 'https://openapi-ueaz.tuyaeu.com';
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!deviceId || !action || (action !== 'on' && action !== 'off')) {
    ...
    await supabase.from('device_state').upsert({
      device_id: deviceId,
      kind: 'light',
      status: validAction === 'on' ? 'on' : 'off',
      updated_at: new Date().toISOString()
    }, { onConflict: 'device_id' });

    return jsonBody(200, { ok: true, deviceId, action: validAction, status: validAction === 'on' ? 'on' : 'off' });
  } catch (error) {
