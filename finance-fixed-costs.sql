...
alter table public.shared_fixed_costs alter column created_by drop not null;
...
create policy "Create shared fixed costs" on public.shared_fixed_costs for insert to authenticated with check (created_by is null or created_by = auth.uid());
create policy "Update shared fixed costs" on public.shared_fixed_costs for update to authenticated using (created_by is null or created_by = auth.uid()) with check (created_by is null or created_by = auth.uid());
create policy "Delete own shared fixed costs" on public.shared_fixed_costs for delete to authenticated using (created_by is null or created_by = auth.uid());
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'shared_fixed_costs'
  ) then
